 ...
 * 5. custom_attributes 全部帶入，使用關聯陣列以 attribute_code 為 key，不再僅取 store_id 為 0 的資料，
 *    而是抓取全部 store_id 的資料，並將 store_id 一併保存。
...
foreach ($products as $prod) {
    ...
    // custom_attributes 全部帶入，使用關聯陣列以 attribute_code 為 key，不再僅取 store_id = 0 的資料，而是抓取全部 store_id 的資料
    ...
    foreach ($results as $row) {
        ...
        if (in_array($attrCode, $attributesToConvert, true) && is_numeric($attrValue)) {
            ...
            // 從 M1 的 eav_attribute_option_value 取 label (依據對應的 store_id)
            $sqlM1Label = "
                SELECT value
            ...
            if ($m1Label !== false) {
                // 在 M2 的 eav_attribute 取該 attribute_code 的 attribute_id (entity_type_id=4)
                $sqlM2Attr = "
                    SELECT attribute_id
                    FROM eav_attribute
                ...
                if ($m2AttributeId) {
                    // 根據 attribute_id 與 label 從 M2 的 eav_attribute_option 找出對應的 option_id
                    $sqlM2Option = "
                        SELECT oa.option_id
                    ...
                    // 如果有找到，就以 M2 option_id 覆蓋原 M1 的值
                    if ($m2OptionId) {
    ...
    // 將多筆同 attribute_code 的資料展開成一維陣列
    $customAttributes = [];
